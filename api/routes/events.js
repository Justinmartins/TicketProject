...
const defaultPinata = require('../infrastructure/pinata');

function createEventsRouter(eventService, purchaseRouter, pinata = defaultPinata) {
  const router = Router();
  ...
  router.post('/', upload.single('banner'), async (req, res) => {
    try {
      const data = { ...req.body };
      if (req.file) {
        const hash = await pinata.uploadFileToIPFS(req.file.buffer, req.file.originalname, req.file.mimetype);
  ...
  router.get('/:id', (req, res) => {
    try {
      res.json(eventService.getEvent(Number(req.params.id)));
    } catch (err) {
  ...
  router.post('/:id/tiers', upload.single('image'), async (req, res) => {
    try {
      ...
      if (req.file) {
        const imageHash = await pinata.uploadFileToIPFS(req.file.buffer, req.file.originalname, req.file.mimetype);
        const metadata = {
          name: data.name,
          description: `Ticket for event #${req.params.id}`,
          image: `https://gateway.pinata.cloud/ipfs/${imageHash}`
        };
        const metaHash = await pinata.uploadJsonToIPFS(metadata);
        data.ticket_uri = `https://gateway.pinata.cloud/ipfs/${metaHash}`;
      }

      const category = await eventService.createTicketCategory(Number(req.params.id), data);
  ...
  router.patch('/:id/tiers/:catId/contract', (req, res) => {
    try {
      const category = eventService.updateCategoryContract(Number(req.params.catId), req.body);
      res.json(category);
    } catch (err) {
      res.status(400).json({ error: err.message });
    }
