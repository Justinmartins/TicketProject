function createEventService(db, chain) {
  ...
  async function createTicketCategory(event_id, data) {
    ...
    const deployment = await chain.deployTicketContract({
      name,
      symbol,
  ...
  async function withdrawRevenue() {
    ...
    for (const address of addresses) {
      try {
        const balance = await chain.getContractBalance(address);
        if (balance > 0n) {
          attemptedCount++;
          await chain.withdrawFromContract(address);
          withdrawnCount++;
        }
      } catch (err) {
  ...
  return { createEvent, getEvent, listEvents, createTicketCategory, updateCategoryContract, withdrawRevenue };
}
